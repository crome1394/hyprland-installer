...
SessionManagementScreen {
    ...
    // Set from Main.qml while SDDM authenticates — blocks double-submit without
    // disabling the whole StackView (which previously froze the greeter).
    ...
    *
    * UI busy-state / re-enable on failure is handled in Main.qml so a failed
    * password cannot leave the greeter permanently disabled (freeze).
    */
    function startLogin() {
        if (loginBusy) {
            return
    ...
    function resetAfterLoginFailure() {
    ...
    Input {
        ...
        placeholderText: (!config.PasswordFieldPlaceholderText || config.PasswordFieldPlaceholderText === "Password")
            ? i18nd("plasma_lookandfeel_org.kde.lookandfeel", "Password")
            : config.PasswordFieldPlaceholderText
