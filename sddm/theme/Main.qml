...
Item {
    ...
    // True while SDDM is authenticating. Ensures the greeter always recovers
    // after a wrong password (disabling mainStack without a guaranteed unlock
    // was freezing the UI).
    ...
    function setLoginBusy(busy) {
        ...
        // Keep mainStack.enabled always true so a failed attempt can never
        // soft-lock the greeter. Only dim the user list / footer and mark the
        // active Login page busy (blocks double-submit).
        footer.enabled = !busy
        if (userListComponent) {
            userListComponent.loginBusy = busy
            if (userListComponent.userList) {
                userListComponent.userList.opacity = busy ? 0.5 : 1
            }
        }
        const page = mainStack.currentItem
        if (page && page !== userListComponent && page.loginBusy !== undefined) {
            page.loginBusy = busy
        }
        if (busy) {
    ...
    function recoverFromLoginFailure() {
        setLoginBusy(false)
        // Ensure stack stays interactive even if RejectPasswordAnimation ran
        mainStack.enabled = true
        footer.enabled = true
        notificationMessage = i18nd("plasma_lookandfeel_org.kde.lookandfeel", "Login Failed")
        // Prefer the visible stack page (user list or "Other…" prompt)
        const page = mainStack.currentItem
        if (page && typeof page.resetAfterLoginFailure === "function") {
            page.resetAfterLoginFailure()
        } else if (userListComponent && userListComponent.mainPasswordBox) {
            userListComponent.mainPasswordBox.selectAll()
    ...
    RejectPasswordAnimation {
        ...
        target: mainStack
        // Belt-and-suspenders: never leave the stack disabled after the shake.
        onFinished: {
            mainStack.enabled = true
            footer.enabled = true
        }
    ...
    MouseArea {
        ...
        QQC2.StackView {
            ...
            initialItem: Login {
                ...
                actionItems: [
                    ActionButton {
                        icon.name: "/usr/share/sddm/themes/Gently-Blur-SDDM-6/assets/suspend_primary.svg"
                        text: i18ndc("plasma_lookandfeel_org.kde.lookandfeel", "Suspend to RAM", "Sleep")
                        onClicked: sddm.suspend()
                        enabled: sddm.canSuspend && !root.loginInProgress
                    },
                    ActionButton {
                        icon.name: "/usr/share/sddm/themes/Gently-Blur-SDDM-6/assets/restart_primary.svg"
                        text: i18nd("plasma_lookandfeel_org.kde.lookandfeel", "Restart")
                        onClicked: sddm.reboot()
                        enabled: sddm.canReboot && !root.loginInProgress
                    },
                    ActionButton {
                        icon.name: "/usr/share/sddm/themes/Gently-Blur-SDDM-6/assets/shutdown_primary.svg"
                        text: i18nd("plasma_lookandfeel_org.kde.lookandfeel", "Shut Down")
                        onClicked: sddm.powerOff()
                        enabled: sddm.canPowerOff && !root.loginInProgress
                    },
                    ActionButton {
                        icon.name: "/usr/share/sddm/themes/Gently-Blur-SDDM-6/assets/user-prompt"
        ...
        Component {
            ...
            Login {
                ...
                actionItems: [
                    ...
                    ActionButton {
                        icon.name: "/usr/share/sddm/themes/Gently-Blur-SDDM-6/assets/switch_primary.svg"
    ...
    // Safety net: if authentication hangs or loginFailed is never delivered,
    // unlock the greeter so the user is not stuck with a frozen login screen.
    Timer {
        id: loginWatchdog
        interval: 8000
        repeat: false
